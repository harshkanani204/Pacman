...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for pac1,pac2 in variableDomainsDict[PAC]:
        for ghost1,ghost2 in variableDomainsDict[GHOST0]:
            distance = abs(ghost1-pac1) + abs(ghost2-pac2)
            if distance + MAX_NOISE not in variableDomainsDict[OBS0]:
                variableDomainsDict[OBS1].append(distance + MAX_NOISE)
                variableDomainsDict[OBS0].append(distance + MAX_NOISE)
            if distance - MAX_NOISE >= 0 and distance - MAX_NOISE not in variableDomainsDict[OBS0]:
                variableDomainsDict[OBS0].append( distance - MAX_NOISE)
                variableDomainsDict[OBS1].append( distance - MAX_NOISE)
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for i in eliminationOrder:
        	...
        	if( type(joinedFactor) == int ): 
        		continue
        	if( len( joinedFactor.unconditionedVariables() ) > 1):
        		incrementallyMarginalizedJoint = eliminate(joinedFactor, i)
        		currentFactorsList.append(incrementallyMarginalizedJoint)
       	
       	if (len( currentFactorsList[0].unconditionedVariables() )) == 1:
            if len(currentFactorsList)==1:
       		    return currentFactorsList[0]
